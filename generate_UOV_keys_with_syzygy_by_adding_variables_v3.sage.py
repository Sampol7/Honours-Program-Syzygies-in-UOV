...
# This file was *autogenerated* from the file generate_UOV_keys_with_syzygy_by_adding_variables_v3.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_0p9 = RealNumber('0.9'); _sage_const_2 = Integer(2); _sage_const_5 = Integer(5); _sage_const_3 = Integer(3)
...
def generate_keys_with_syzygy(k, n, m, random_ratio=_sage_const_0p9 , order="deglex"):
    """
    ...
    """
    R = PolynomialRing(k, ["x%d" % i for i in range(_sage_const_1 , n+_sage_const_1 )] + [f'a_{i}_{j}' for i in range(m) for j in range(n)] + [f'b_{i}_{j}_{l}' for i in range(m) for j in range(n) for l in range(n)])
    R = PolynomialRing(k, ["x%d" % i for i in range(_sage_const_1 , n+_sage_const_1 )])
    x_vars = [SR.var(f'x{i}') for i in range(_sage_const_1 , n+_sage_const_1 )]
    ...
    while L is None:
        ...
        for i in range(m):
            for j in range(n):
                for l in range(n):
                    ...
                    if j >= (n - m) and l >= (n - m):
                        random_var_eqns.append(var_b == _sage_const_0 )
                    else:
        ...
        def construct_linear_system_from_syzygy(eq):
            ...
            # Iterate through the terms of the expanded polynomial
            for term in polynomial.operands():
                coeff = term.subs({x:_sage_const_1  for x in x_vars})  # Calculate the coefficient
                monomial_part = term/coeff
                monomial_part = monomial_part.expand()  # Ensure it's in canonical form
            ...
            linear_system = [coeff == _sage_const_0  for coeff in monomial_dict.values()]
            return linear_system
        ...
        F_with_substituted_solutions = substitute_solutions_in_F(F, solutions[_sage_const_0 ], b_varss)
