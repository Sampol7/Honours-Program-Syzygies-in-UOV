...
def time_and_verify_key_gen(q_range, n_range, m_range):
    import time
    time_dict = {}
...
#time_dict = time_and_verify_key_gen(10, 7, 6)
...
def time_key_gen(q,n,m):
    ...
    while zero:
        F, L = generate_keys_with_dependency(k, n, m)
        F_polys = symbolic_to_polynomial2(F, R, k)    #ZeroDivisionError: inverse of Mod(0, 2) does not exist
        if not any(poly == 0 for poly in F_polys):
            zero=False
    P, T = private_to_public(F_polys, k, n, m)
    ...
    return F_polys, L, P, T, time_taken, cpu_time
