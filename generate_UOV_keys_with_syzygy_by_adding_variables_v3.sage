...
def generate_keys_with_syzygy(k, n, m, random_ratio=0.9, order="deglex"):
    """
    ...
    while L is None:
        ...
        def substitute_and_remove(constraints, variables):
            ...
            for constraint in constraints:
                if str(constraint.lhs()).startswith('b_') and constraint.lhs().is_symbol() and constraint.rhs().is_constant():
                    ...
                    variables.remove(constraint.lhs())
                else:
            ...
            return updated_constraints, variables

        variables = a_varss+b_varss
        linear_system, variables = substitute_and_remove(linear_system, variables)
        print("linear_system")
        print(linear_system)
        print("variables")
        print(variables)
        ...
        solutions = solve(linear_system, variables)
        print(solutions)
