...
# This file was *autogenerated* from the file distinguish_OUV_from_random.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_7 = Integer(7); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_0p2 = RealNumber('0.2'); _sage_const_2p0 = RealNumber('2.0')
...
num_keys = _sage_const_2 
field = GF(_sage_const_7 )
num_vars = _sage_const_4 
num_polys = _sage_const_3 
...
def is_uov_key(keys):
    ...
    num_cols = all_coeffs.shape[_sage_const_2 ]  # Aangepast naar de juiste dimensie
    correlations = []

    for i in range(num_cols):
        for j in range(i + _sage_const_1 , num_cols):
    ...
    if avg_corr > _sage_const_0p2 :  # Deze drempelwaarde kan worden aangepast na testing
        return "UOV"
    else:
        return "Random"
...
def is_uov_key_oil_vinegar(keys, vinegar_vars, oil_vars):
    ...
    if total_vars <= _sage_const_0 :
        raise ValueError("Het totaal aantal olie- en azijnvariabelen moet positief zijn.")

    oil_azijn_interacties = _sage_const_0 
    oil_oil_interacties = _sage_const_0 
        ...
        for matrix in key:
            ...
            for i in range(total_vars):
                for j in range(total_vars):
                    ...
                    if matrix[i][j] != _sage_const_0 :  # Alleen als de interactie niet nul is
                        if i < oil_vars and j < oil_vars:
                            # Olie-olie interactie
                            oil_oil_interacties += _sage_const_1 
                        elif (i < oil_vars and j >= oil_vars) or (i >= oil_vars and j < oil_vars):
                            # Olie-azijn interactie
                            oil_azijn_interacties += _sage_const_1 

    # Bereken de verhouding olie-azijn vs olie-olie interacties
    if oil_oil_interacties == _sage_const_0 :  # Vermijd deling door nul
        oil_oil_interacties = _sage_const_1 
    ...
    if interactie_ratio > _sage_const_2p0 :  # Deze drempelwaarde kan aangepast worden na verdere analyse
        return "UOV"
    else:
        return "Random"
...
def is_uov_key_solution_space_dimension(generated_keys, num_oil, num_polys):
    ...
    for key in generated_keys:
        ...
        for matrix in key:
            ...
            x = [var(f'x{i+_sage_const_1 }') for i in range(num_oil + num_polys)]
