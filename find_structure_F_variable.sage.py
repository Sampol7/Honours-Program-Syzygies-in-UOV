...
# This file was *autogenerated* from the file find_structure_F_variable.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_1433 = Integer(1433); _sage_const_2 = Integer(2); _sage_const_5 = Integer(5); _sage_const_10 = Integer(10); _sage_const_6 = Integer(6); _sage_const_25 = Integer(25)#############################################
###Experiment 1
#############################################
...
for run in range(runs):
    ...
    R = PolynomialRing(k, ["x%d" % i for i in range(_sage_const_1 , n+_sage_const_1 )] + [f'f_{l}_{i}_{j}' for l in range(m) for i in range(n) for j in range(i, n)])
    x = R.gens()

    # Step 1: Generate m random linear polynomials L_i with coefficients in k
    #L = [sum([R(i+1) * x[i] for i in range(n)]) for _ in range(m)]
    L = [sum([((l+_sage_const_1 )*_sage_const_10 +(i+_sage_const_1 )) * x[i] for i in range(n)]) for l in range(m)]
    ...
    def linear_system_to_matrix(equations, variables, k):
        ...
        print(((n+_sage_const_2 )*(n+_sage_const_1 )*n/_sage_const_6 ))
        print(m*((n+_sage_const_1 )*n)/_sage_const_2 )
