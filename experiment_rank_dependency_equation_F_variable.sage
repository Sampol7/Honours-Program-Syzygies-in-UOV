...
for run in range(runs):
    ...
    R = PolynomialRing(k, ["x%d" % i for i in range(1, n+1)] + [f'f_{l}_{i}_{j}' for l in range(m) for i in range(n) for j in range(i, n)])
    x = R.gens()

    # Step 1: Generate m random linear polynomials L_i with coefficients in k
    #L = [sum([R(i+1) * x[i] for i in range(n)]) for _ in range(m)]
    L = [k.random_element() for _ in range(m)]
    #L = [sum([k.random_element() * x[i] * x[j] for i in range(n) for j in range(i,n)]) for _ in range(m)]
    #print("L")
    #print(L)

    f_vars = var([f'f_{l}_{i}_{j}' for l in range(m) for i in range(n) for j in range(i, n)])
    ...
    def construct_linear_system_from_dependency(eq, n, m):
        ...
        f_vars = var([f'f_{l}_{i}_{j}' for l in range(m) for i in range(n) for j in range(n)])
