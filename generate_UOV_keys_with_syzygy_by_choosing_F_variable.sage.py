...
# This file was *autogenerated* from the file generate_UOV_keys_with_syzygy_by_choosing_F_variable.sage
...
def generate_keys(k, n, m, order="deglex"):
    R = PolynomialRing(k, ["x%d" % i for i in range(_sage_const_1 , n+_sage_const_1 )] + [f'b_{l}_{i}_{j}' for l in range(m) for i in range(n-m) for j in range(n)])
    M = MatrixSpace(k, n)
