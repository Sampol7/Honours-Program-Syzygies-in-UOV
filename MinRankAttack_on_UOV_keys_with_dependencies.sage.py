...
# This file was *autogenerated* from the file MinRankAttack_on_UOV_keys_with_dependencies.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_35 = Integer(35); _sage_const_3 = Integer(3); _sage_const_17 = Integer(17); _sage_const_7 = Integer(7); _sage_const_10 = Integer(10)
import hashlib
...
class MinRankAttack(object):
    ...
    def run_groebner(self):
        ...
        delta = (self.__MR.r - self.__MR.eta) * (self.__MR.n - self.__MR.r) +             self.__MR.m
        if delta > _sage_const_0 :
            PS = PS.subs({v: _sage_const_0  for v in R.gens()[:delta]})

        # Form the ideal, making sure everything's OK.  The field equations
        # are added to the ideal to make sure that it is radical.
        I = R.ideal(*PS) +             sage.rings.ideal.FieldIdeal(R)
    ...
    def __run_xl(PS, D):
        def generate_monomials(R, deg):
            ...
            return [R({tuple(degs): _sage_const_1 })
                    for degs in WeightedIntegerVectors(
                            deg, [_sage_const_1  for _ in range(R.ngens())]
                    )]

        def solve(PS, offset=_sage_const_0 , soln=None):
            ...
            for i in range(offset, len(PS)):
                ...
                for root in roots:  # Try to solve for each root.
                    new_soln = soln.copy()
                    new_soln[PS[i].variable(_sage_const_0 )] = root
        ...
        PS = PolynomialSequence(
            ...
            chain(
                [m * p
                 for p in PS
                 for m in generate_monomials(PS.ring(), D - _sage_const_2 )],
                PS
            )
        )
    ...
    def run_xl(self, D):
        ...
        """
        assert D >= _sage_const_2 
        system, R = self.__generate_MQ_system(self.__MR)
        return self.__run_xl(PolynomialSequence(R, system), D)

    def __str__(self):
        """Pretty printing."""
        return "MinRankAttack object.  Associated MR instance: " +             str(self.__MR) + "."
...
class Prover(object):
    ...
    def receive_query(self, verifier, Q):
        ...
        """
        if Q not in (_sage_const_0 , _sage_const_1 , _sage_const_2 ):
            raise ValueError(
        ...
        elif Q == _sage_const_1  or Q == _sage_const_2 :
            verifier.receive_response(Q, self.__STX, self.__beta[Q - _sage_const_1 ])

    def __str__(self):
        """Pretty printing."""
        return "(Naive) Prover object.  Associated MR instance: " +             str(self._MR) + "."
...
class LegitimateProver(Prover):
    ...
    def __str__(self):
        """Pretty printing."""
        return "LegitimateProver object.  Associated MR instance: " +             str(self._MR) + "."
...
class Verifier(object):
    ...
    def send_query(self, prover):
        ...
        prover.receive_query(self, ZZ.random_element(_sage_const_0 , _sage_const_3 ))
        # Reset.
        self.__has_hashes = False
    ...
    def receive_response(self, Q, *args):
        ...
        if Q == _sage_const_0 :
            ...
            # Check rank of S * M * T.
            if (ID[_sage_const_1 ] - ID[_sage_const_0 ]).rank() != self.__MR.r:
                raise AuthenticationError("Invalid rank.")
        elif Q == _sage_const_1  or Q == _sage_const_2 :
            # Unpack arguments.
            (S, T, X) = args[_sage_const_0 ]
            beta = args[_sage_const_1 ]
            ...
            # Check STX hash.
            if sha256_hash(args[_sage_const_0 ]) != self.__STX_hash:
            ...
            if Q == _sage_const_1 :
                if sha256_hash(temp) != self.__ID_hash[_sage_const_0 ]:
                    raise AuthenticationError("Invalid ID[0] hash.")
            else:
                if sha256_hash(temp - S * self.__MR.offset_matrix * T) !=                        self.__ID_hash[_sage_const_1 ]:
                    raise AuthenticationError("Invalid ID[1] hash.")

    def __str__(self):
        """Pretty printing."""
        return "Verifier object.  Associated MR instance: " +             str(self.__MR) + "."
...
class MinRankInstance(object):
    ...
    def __init__(self, m, eta, n, r, q):
        ...
        while True:
            ...
            if self.__alpha[-_sage_const_1 ] != _sage_const_0 :
                break

        # Deterministically generate the last part of the instance.
        self.__Ms.append(
            ((self.__M + self.__offset_matrix -
              linear_combination(self.__alpha[:-_sage_const_1 ], self.__Ms)) /
             self.__alpha[-_sage_const_1 ])
        )
        print("self.__Ms:", self.__Ms)
    ...
    for i in range(rounds):
        ...
        try:
            ...
            print(("Authentication unsucessful.\n"
                   "Failed at round: {:02d}\n"
                   "Reason         : {}").format(i + _sage_const_1 , str(e)))
            return False
...
set_random_seed(_sage_const_17 )
