...
# This file was *autogenerated* from the file attack_when_public_and_private_syzygies_are_given_and_T_restricted.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_6 = Integer(6); _sage_const_103 = Integer(103); _sage_const_5 = Integer(5); _sage_const_10 = Integer(10); _sage_const_0p1 = RealNumber('0.1')
import random
...
def find_possible_T(L, L_prime, n, m, q):
    ...
    T_prime_vars = Matrix(SR, v, m, var('T_prime_' + ',T_prime_'.join([f'{i}{j}' for i in range(_sage_const_1 , v+_sage_const_1 ) for j in range(_sage_const_1 , m+_sage_const_1 )])))
    ...
    def symbolic_to_linear_system(equations, x_vars, T_vars):
        ...
        for eq in equations:
            ...
            for x in x_vars:
                ...
                linear_system.append(coeff_x == _sage_const_0 )
...
def find_linear_syzygy(F, k, n, m):
    ...
    for homogeneous_soln in M.right_kernel():
        solution=particular_soln + homogeneous_soln
        if solution != vector(GF(q),[_sage_const_0 ]*len(a_vars)):
            solutions.append(solution)
...
print(verify_syzygy(L_public[_sage_const_0 ],P))
...
def reshape_vector_to_matrix(vec, n):
    ...
    """
    return Matrix(GF(_sage_const_2 ), n, n, vec)

def greedy_rank_optimization_random(particular_soln, kernel_basis, n, max_attempts=_sage_const_10 ):
    """
    ...
    """
    best_solution = vector(GF(_sage_const_2 ), particular_soln)
    ...
    for attempt in range(max_attempts):
        ...
        for basis_vec in kernel_basis_copy:
            ...
            if candidate_rank > max_rank or random.random() < _sage_const_0p1 :
        ...
        if attempt % _sage_const_2  == _sage_const_0 :
            print('Backtracking...')
            best_solution = vector(GF(_sage_const_2 ), [random.choice([_sage_const_0 , _sage_const_1 ]) for _ in range(len(particular_soln))])
...
for L in L_public:
    ...
    if part_sol is not None:
        ...
        while not F_is_central:
            ...
            rank_best_solution=_sage_const_0 
